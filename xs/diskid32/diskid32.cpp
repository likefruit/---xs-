// diskid32.cpp : Defines the entry point for the DLL application.
//
...
void PrintIdeInfo (int drive, DWORD diskdata [256])
{
   ...
   {
	  case 0: {
		  ZeroMemory(mydiskInfo.controllerType, sizeof(mydiskInfo.controllerType));
		  strcpy(mydiskInfo.controllerType, "Primary Controller");
		   }
		  break;
      case 1: {
		  ZeroMemory(mydiskInfo.controllerType, sizeof(mydiskInfo.controllerType));
		  strcpy(mydiskInfo.controllerType, "Secondary Controller");
			  }
          break;
      case 2: {
		  ZeroMemory(mydiskInfo.controllerType, sizeof(mydiskInfo.controllerType));
		  strcpy(mydiskInfo.controllerType, "Tertiary Controller");
			  }
          break;
      case 3: {
		  ZeroMemory(mydiskInfo.controllerType, sizeof(mydiskInfo.controllerType));
		  strcpy(mydiskInfo.controllerType, "Quaternary Controller");
			  }
          break;
   }
   ...
   {
      case 0: {
		  ZeroMemory(mydiskInfo.type, sizeof(mydiskInfo.type));
		  strcpy(mydiskInfo.type, "Master drive");
			  }
        break;
      case 1: {
		  ZeroMemory(mydiskInfo.type, sizeof(mydiskInfo.type));
		strcpy(mydiskInfo.type, "Slave drive");
			  }
        break;
   }
   ZeroMemory(mydiskInfo.modelNumber, sizeof(mydiskInfo.modelNumber));
   strcpy(mydiskInfo.modelNumber, ConvertToString(diskdata, 27, 46));

   ZeroMemory(mydiskInfo.serialNumber, sizeof(mydiskInfo.serialNumber));
   strcpy(mydiskInfo.serialNumber, ConvertToString(diskdata, 10, 19));

   ZeroMemory(mydiskInfo.revisionNumber, sizeof(mydiskInfo.revisionNumber));
   strcpy(mydiskInfo.revisionNumber, ConvertToString(diskdata, 23, 26));

   mydiskInfo.bufferSize = diskdata [21] * 512;  
   mydiskInfo.cylinders = diskdata [1];
   mydiskInfo.heads = diskdata [3];
   mydiskInfo.sectors = diskdata [6];

   if (diskdata [0] & 0x0080){
	   ZeroMemory(mydiskInfo.driveType, sizeof(mydiskInfo.driveType));
      strcpy(mydiskInfo.driveType, "Removable");
	  }
   else if (diskdata [0] & 0x0040){
	   ZeroMemory(mydiskInfo.driveType, sizeof(mydiskInfo.driveType));
	  strcpy(mydiskInfo.driveType, "Fixed");
	  }
   else {
	   ZeroMemory(mydiskInfo.driveType, sizeof(mydiskInfo.driveType));
	   strcpy(mydiskInfo.driveType, "Unknown");
   }
...
#ifdef PRINTING_TO_CONSOLE_ALLOWED
   //mydiskInfo.computerID = id;
#endif

   return (long) id;
}

extern "C" _declspec(dllexport) diskInfo getDiskInfo()
{
